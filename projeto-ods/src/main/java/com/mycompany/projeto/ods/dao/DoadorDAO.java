 ...
 * O SQLite não possui tipos DATE e TIME nativos como o MySQL. Ele armazena
 * datas
 * e horas como TEXT no formato ISO 8601 ("yyyy-MM-dd" para data, "HH:mm:ss"
 * para hora).
 * Por isso, todos os setDate()/setTime() do código original foram substituídos
 * por
 * setString() usando LocalDate.toString() e LocalTime.toString(), que produzem
 * exatamente esse formato ISO. Na leitura, getString() + LocalDate.parse() faz
 * o
 * caminho inverso.
 *
 * TRATAMENTO DE EXCEÇÕES:
 * System.err.println foi removido. Métodos que modificam dados (salvar,
 * atualizar,
...
public class DoadorDAO {
    ...
    /**
     * Atualiza os dados editáveis do doador: nome, tipo sanguíneo e, opcionalmente,
     * senha.
     *
     ...
     * EditarPerfilScreen também deve exibir o CPF como somente-leitura
     * (setEditable(false)).
     *
     * Lógica da senha:
     * - Se a senha no objeto já começa com "$2a$", é um hash BCrypt existente → não
     * refaz o hash.
     * - Se for texto puro (usuário digitou nova senha) → gera novo hash antes de
     * salvar.
     *
     ...
     *
     * @return o Doador autenticado, ou null se CPF não encontrado ou senha
     *         incorreta
     * @throws BancoDadosException se ocorrer falha de conexão (para que a
     *                             LoginScreen
     *                             possa exibir "Erro ao conectar com o banco" ao
     *                             usuário)
     */
